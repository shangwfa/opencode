...
| 2026-09-16 | `person-model-connect`（feat/opencode-1.18.31 工作区：个人模型隔离 + 公共优先 + provider 脱敏 + autokeepalive，本地 PG + 远端 K8s 沙箱；实例 12:49 UTC 启动，本日全量用例执行后） | T-WDT.1/2/3 | ✅ 3/3 | T-WDT.1 ✅（新增超龄 running=0，历史存量 0）；T-WDT.2 ✅（活超时 maxGap=0ms，无 watchdog 标记样本，跨实例遗留 0/LEASE 孤儿 0）；T-WDT.3 ✅（lsp/todowrite 标记 0 观察项；bash watchdog 0/3364 误杀率 0——当日 sse/e2e/stability 全量用例的 bash 调用无一误标记） |
