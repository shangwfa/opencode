...
| 2026-09-14 | **全量复测**（镜像 `hitl-cbf2276a-wip2`（含 pgJsonb/LEASE_TOOLS/偏离修复），本地 PG + 远端 K8s 沙箱） | ✅ 13 pass / 1 单测锁定 / 1 skip | 单测三层 33/33（检测器 29 + processor 4）。集成：T42.1.1 ✅（2 真实+repeat block+换方法，另顺带展示连败 fail 通道）/ T42.1.2 ✅（block 后模型停止+会话可用；fatal 链路本轮模型两次极端诱导均不配合——1 次即停/新 run 重置，**由单测 LoopAbortedError 无条件 stop + 09-08 集成实测覆盖**）/ T42.1.3 ✅（read 跨 3 消息第 3 次 block）/ T42.1.4 ✅（block 后改读 b.txt 立即 completed）/ T42.1.5 ✅（本轮为「并行全放行」形态：同轮 3 read 全真实+第 4 block；按 AI SDK 时序波动，两种形态均在用例书预期框架内）/ T42.2.1 ✅（f1~f3 连败+第 4 fail-block）/ T42.2.2 ✅（g1 重试 fail-block；fixed.txt **真实执行**=自愈放行——文件本身因沙箱重建丢失属环境偶发）/ T42.2.3 ✅（5 命令 completed+exit 7~10/0+零标记；**首跑曾因远端沙箱资源耗尽（20 个 keep-alive 残留测试沙箱挤占，Sandbox create timed out→Effect fail 误触 3 连败 block——检测器对真实失败的行为正确），kill-sandbox 清理 20 个后恢复**）/ T42.2.4 ⚠️ 单测锁定（本轮模型用 bash `timeout 5` 自救绕过 FIFO 诱导未产生 read 调用；interrupt 不计数分支由 anti-loop-tools abort 用例钉死 + 09-08 集成定性：watchdog 只杀孤儿）/ T42.3.1 ✅（observe+disable env 容器下 block 照常）/ T42.4.1 ✅（2 工具 0 标记）/ T42.4.2 ✅（SSE permission.asked=0+新机制 block=1）/ T42.4.3 ✅（run2 同命令 completed 零标记）/ T42.4.4 ✅（顺序第 3 task repeat block）/ T42.4.6 ✅（双会话并发 A block=2 B block=1 互不影响）/ T42.4.5 ⏭️ SKIP（无 MCP 环境） |
