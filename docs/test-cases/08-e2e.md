...
curl -s --max-time 120 -X POST "$BASE/session/$SID/message" \
  ...
  | python3 -c "import json,sys;d=json.load(sys.stdin);[print(p.get('text','')[:200]) for p in d.get('parts',[]) if p.get('type')=='text']"
```
...
curl -s -X POST "$BASE/session/$SID/exec" \
  -H 'Content-Type: application/json' \
  -d '{"command":"find /workspace/myapp /workspace/tests -type f -name \"*.py\" | sort"}' \
...
curl -s -X POST "$BASE/session/$SID/exec" \
  -H 'Content-Type: application/json' \
  -d '{"command":"cd /workspace && python3 -c \"from myapp.main import main; main()\""}' \
  | python3 -c "import json,sys;d=json.load(sys.stdin);print('exit:',d.get('exitCode'));print('stdout:',d.get('stdout','').strip())"
# 期望输出: hello
```
...
curl -s --max-time 60 -X POST "$BASE/session/$SID/message" \
  ...
  | python3 -c "import json,sys;d=json.load(sys.stdin);[print(p.get('text','')[:100]) for p in d.get('parts',[]) if p.get('type')=='text']"
