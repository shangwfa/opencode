...
> 借鉴 Headroom 的内容级压缩思路：在每次 LLM 请求前，对**大体积 tool output** 做确定性压缩（JSON / 日志 / 通用文本），原文存入 CCR 存储（PG `storage_data` 表），上下文中只留短 preview + `[ccr:<hash>]` marker，模型可随时调用 `ccr_retrieve` 取回原文。历史级老化由内置 compaction + history 文件承担（见 `session-compaction-history.md`），CCR 主攻高频大输出的确定性瘦身。

## 功能背景与效果

内置 compaction 整体摘要以「历史消息」为对象；但 agent 会话中 token 大头往往是**单条 tool output**（read 全文件、bash 长日志、glob/mcp 大 JSON）。CCR 在消息视图层对这类输出做内容感知压缩：
...
| Savings 观测口径                                   | 两级观测：① **per-turn 实时日志**——每次 transform 输出一行 `[ccr] turn: messages=N window=N created=N reused=N compressed=N orig=N comp=N saved=N% images=N took=Nms`（saved 连续下滑=compressor 漂移；reused 向 0 塌缩=marker 字节失配、prefix cache 每轮失效）；② **周级 PG 聚合**——不实现独立 savings 功能（Headroom 的 jsonl 账本+定价 CLI）：PG CCR entries 本身就是账本（并发安全、跨实例、随 session 清理），聚合用 SQL，定价人工乘 `opencode.jsonc` 固定模型单价。**每周观测 SQL**：`SELECT count(*), sum((data->>'originalTokens')::numeric) orig, sum((data->>'compressedTokens')::numeric) comp, round(100*(1-sum((data->>'compressedTokens')::numeric)/sum((data->>'originalTokens')::numeric)),1) pct, sum((data->>'retrievalCount')::numeric) rets FROM storage_data WHERE key LIKE 'plugin/ccr/%';` 缓存健康：`近7天 cache_read/(cache_read+input)`，突降=transform 链漂移警报（当前基线 93.4%） |
...
| L11 兼容     | T-CCR.12 | （历史）曾验证 CCR 与 DCP 同开的注册顺序；DCP 已于 2026-09-04 移除，CCR 独立运行                                                               | n/a（DCP 已移除）                     |
...
> **镜像默认开启**：Dockerfile 已内置 `ENV OPENCODE_CCR_ENABLED=true`，容器无需额外传参。
...
- 压缩发生在请求视图层：`session.messages` API 返回的仍是 PG 原文。
- 日志/文本截断保留 head+tail 与 error 行，中间内容需 retrieve 取回——模型按 marker 提示自行决策。
- CCR preview 对 provider 是稳定前缀字节，但首次替换该输出时仍会使该点之后缓存失效。
...
| 2026-09-08 | 工作树 | per-turn 观测日志落地：transform 收敛为单行 `[ccr] turn:` 汇总（含 created/reused 拆分、saved%、耗时），`store.replace` 返回 `{replacement, origin}` 区分新建/复用；CCR 5 套件 97 pass / 0 fail，typecheck 59 err vs 基线 60（不新增）；另核实 compaction「limit.input 无 headroom」bug 已被 upstream 修复（`usable()` 双分支均减 reserved，3 个回归测试通过），溢出硬兜底已存在于 `processor.ts` halt()（ContextOverflowError → needsCompaction） |
