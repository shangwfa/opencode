...
| 2026-09-16 | 本地 PG + 远端 K8s 沙箱，镜像 `person-model-connect`（feat/opencode-1.18.31 工作区：个人模型隔离 + 公共优先 + provider 脱敏 + autokeepalive） | PASS | **全量回归 T-FILE-01~74 共 37 断言全过**（首跑 11 例 FAIL 均为执行脚本 exec 构造的 shell 花括号 bug 连锁——exec 验证全空导致 vdir 未建、create 误报 200；修正后复验全过，T-FILE-05 干净复现确认 `create` 打目录 502 且目录内容保留）。要点：T-FILE-72 文件树 **0s** 返回（runDetached 通道）；T-FILE-42 kill-sandbox + boot 重建后 PVC SHA 一致；T-FILE-44 app 模式双 session 共享卷读到同一文件；autokeepalive 下 mkdir/create 懒创建路径不变。执行提示：exec 请求体构造勿用内联 `python -c` 双引号嵌套（花括号被 shell 吞），用辅助脚本文件；特殊字符路径须 URL 编码 |
