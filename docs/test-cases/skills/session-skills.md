...
> 复测记录（2026-09-06，merge upstream/dev v1.18.29 后，镜像 `t0906-merged-1.18.29`，组合 3，模型 `opencode/muse-spark-1.3-contributor-free`）：T15.1 ✅（skill 创建 resources=[]，spark 主动触发 `skills:["reviewer"]` 明确声明使用 reviewer skill 并按格式审查）；T15.2 ✅（bundle 含 resources + type 字段，触发后模型引用物化路径 `/home/sandbox/.local/share/opencode/session-skills/...`，资源物化机制 T15.7 顺带验证）；T15.3 ✅（删单个 204/清空 204）；T15.6 ✅（同名 upsert 覆盖 content）；T15.8 ✅（不存在的 skill 消息 200 正常降级）；T15.10 ✅（session permission deny skill 后未注入，模型自述无该 skill）。注意：resources 缺 `type` 字段返回 BadRequest（必填校验，正确行为）。单测：`test/skill/skill.test.ts`+`resource.test.ts` 46/46 ✅；`session-skill*.test.ts` 挂 `@opencode/Location` unbound（既有单测基建偏差，同 processor-effect，非 merge 引入，HTTP 集成已覆盖）。
