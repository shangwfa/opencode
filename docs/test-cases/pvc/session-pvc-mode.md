...
| 用例 | 验证维度 | 状态 |
|------|---------|------|
| T38.1 | 默认行为不变（pvcMode=undefined, dir=/workspace） | ✅ (2026-08-02, 2026-08-21) |
| T38.2 | 显式 pvcMode=session | ✅ (2026-08-02, 2026-08-21) |
| T38.3 | app 模式创建 + dir 不暴露 worktree | ✅ (2026-08-02, 2026-08-21) |
| T38.4 | app 缺 appId → 400 | ✅ (2026-08-02, 2026-08-21) |
| T38.5 | app 空白 appId → 400 | ✅ (2026-08-02, 2026-08-21) |
| T38.6 | 非法 pvcMode → 400 | ✅ (2026-08-02, 2026-08-21) |
| T38.7 | appId 路径穿越 → 拒绝 | ✅ (2026-08-02, 2026-08-21) |
| T38.8 | appId 超长(>128) → 拒绝 | ✅ (2026-08-02, 2026-08-21) |
| T38.9 | appId 合法边界字符 → 通过 | ✅ (2026-08-02, 2026-08-21) |
| T38.10 | 同 appId 共享 PVC | ✅ (2026-08-02, 2026-08-21) |
| T38.11 | 不同 appId 隔离 | ✅ (2026-08-02, 2026-08-21) |
| T38.12 | session/app 隔离 | ✅ (2026-08-02, 2026-08-21) |
| T38.18 | PG 持久化 | ✅ (2026-08-02, 2026-08-21) |
| T38.19 | fork 继承 pvcMode/appId + 数据共享 | ✅ (2026-08-02, 2026-08-21) |
| T38.20 | 子任务会话 parent_id 追溯共享 PVC | ✅ (2026-08-02, 2026-08-21) ⚠️ 实测为创建时继承 |
| T38.21 | session 模式不受 app 逻辑影响（回归保护） | ✅ (2026-08-02, 2026-08-21) |
| T38.22 | app 模式 worktree 代码写入位置验证 | ✅ (2026-08-02, 2026-08-21) |
| T38.23 | session 模式 worktree 代码写入位置验证 | ✅ (2026-08-02, 2026-08-21) |
| T38.24 | session 模式 `/vcs/diff` 读取 `/workspace` | ✅ PASS (2026-07-30, 2026-08-21) |
| T38.25 | app 模式 `/vcs/diff` 读取 session worktree | ✅ PASS (2026-07-30, 2026-08-21) |
...
> **2026-08-21 重跑（快照功能合入后回归）**：T38.1–T38.25 **19/19 全 PASS**。环境：SaaS 容器（本地 PG `opencode` 库 + 远端 K8s Sandbox，`VOLUME_TYPE=pvc` 默认模式，镜像 `13b750953b`）。要点：① 创建校验/隔离/继承/worktree E2E/vcs-diff 语义全部与 08-02 一致；② T38.20 复现"创建时直接继承"（child `pvcMode=app`、`appId` 落库）；③ T38.22/23 文档脚本中步骤 4 的 `psql UPDATE session.directory` 为注释，执行时需按 T38.25 模式实际执行（`Bun.spawnSync psql`），否则 `/path`、`pwd` 断言必失败；④ 全程 `session_snapshot` 表 0 新增——**默认 PVC 模式完全不受快照功能影响**。附带观察：批量 DELETE 29 会话时首次循环响应 200 但落库有延迟，重删收敛（3 个二次返回 404，属延迟确认，最终 0 残留）。
