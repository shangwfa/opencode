...
| `OPENCODE_SANDBOX_SNAPSHOT_TTL_SEC` | 否 | `1209600`（14 天） | 快照保留时长；**删除需显式开启 `OPENCODE_SANDBOX_SNAPSHOT_DELETE_ENABLED`**，否则 TTL 过期仅停止恢复、不物理删除 |
| `OPENCODE_SANDBOX_SNAPSHOT_DELETE_ENABLED` | 否 | 关闭 | 远端快照物理删除总开关（TTL GC / superseded / 会话删除联动）。默认关闭防共享 PG 误删；开启后按 TTL 等规则删除 |
