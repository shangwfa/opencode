...
> **会话可见性与 ID 使用约定**：AI 执行子任务（task/subagent）时产生的内部会话带 `parent_id`，
> **对用户不可见**。接入方调用任何 HTTP API 时**只需也只应使用主会话 ID**（无 `parent_id` 的会话）。
> 子会话自动复用主会话的沙箱与 PVC 卷，其生命周期由系统随主会话统一管理，无需（也无法）通过 API 单独操作。

> **上游本地概念的兼容残留（SaaS 下无语义）**：`directory` 与 `worktree` 是上游本地开发工具的概念
> （用户在哪个本地项目目录打开 opencode / git worktree 检出根）。SaaS 形态下两者恒为常量——
> `directory` 恒为 `/workspace`（沙箱工作区）、`worktree` 恒为 `/`（非 git 项目兜底，相关权限豁免
> 逻辑被显式短路）。它们**不承载任何 SaaS 语义，不是访问控制边界，也不是聚合维度**：
> - 业务聚合维度是 `appId`（创建标记 + `?appId=` 列表过滤）
> - 潜在的多租户边界是 `workspace_id`（当前单租户信任模型下未启用数据面校验）
> - 接入方**不应**传 `x-opencode-directory` header（存在仅为上游兼容，传入任意值只是落库字符串）
> 
> 代码保留这两个概念是为了上游合并生命线（`InstanceState` 键控、`containsPath` 等大量引用）；
> SaaS 新功能一律不建立在它们之上。
...
> **基本模型**：沙箱是会话的附属资源，生命周期完全由会话驱动——
> 首次工具调用/命令执行时惰性创建；会话活跃期间自动维持（touch + 命令心跳）；
> 会话空闲后回收；删除会话时联动销毁并清理快照。
> 子会话共享主会话沙箱，不独立管理生命周期。
> **keep-alive 是唯一的显式例外**：设置后沙箱脱离空闲回收，直到 release 或后台兜底扫描（60 分钟无活跃）。
