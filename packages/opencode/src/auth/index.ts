...
function isPublicFileKey(key: string) {
  return !key.includes("/") || key.startsWith("http://") || key.startsWith("https://")
}
    ...
    const all = Effect.fn("Auth.all")(function* (userId?: string) {
      ...
      if (!user) {
        ...
        for (const [key, value] of Object.entries(decoded)) {
          if (isPublicFileKey(key)) pub[key] = value
        }
        return pub
      }
      const prefix = `${user}/`
      const pub: Record<string, Info> = {}
      const personal: Record<string, Info> = {}
      for (const [key, value] of Object.entries(decoded)) {
        if (key.startsWith(prefix)) personal[key.slice(prefix.length)] = value
        if (isPublicFileKey(key)) pub[key] = value
      }
      return { ...personal, ...pub }
    })
...
export const pgLayer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const toRecord = (rows: any[], user: string) => {
      ...
      return { ...personal, ...pub } as Record<string, Info>
    }
