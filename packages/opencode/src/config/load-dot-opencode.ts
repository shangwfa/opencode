...
export interface Interface {
  readonly load: (sessionID: SessionID, directory: string, worktree?: string) => Effect.Effect<Result>
}
...
async function collectSkillResources(skillRoot: string): Promise<SkillResource[]> {
  ...
  for (const rel of candidates) {
    const size = await Filesystem.size(path.join(skillRoot, rel))
    if (size > RESOURCE_MAX) continue
    const content = await Filesystem.readText(path.join(skillRoot, rel)).catch(() => "")
    if (!content) continue
  ...
  Effect.gen(function* () {
    const load = Effect.fn("LoadDotOpencode.load")(function* (sessionID: SessionID, directory: string) {
      ...
      if (agentsMdSvc) {
        ...
        const content = yield* Effect.promise(() => Filesystem.readText(agentsMdPath).catch(() => ""))
        if (content.trim()) {
      ...
      if (agentSvc) {
        const agents = yield* Effect.tryPromise({
          try: () => ConfigAgent.load(dotDir),
          catch: (error) => error,
        }).pipe(
          Effect.catch((error) =>
            Effect.sync(() => {
              skipped.push({ path: "agents", reason: `failed to load agents: ${String(error)}` })
              return {} as Record<string, ConfigAgentV1.Info>
            }),
          ),
        )
        for (const [name, value] of Object.entries(agents)) {
          ...
          yield* agentSvc.upsert(sessionID, toAgentInput(name, value))
      ...
      if (skillSvc) {
        ...
        for (const skillFile of skillFiles) {
          const md = yield* Effect.tryPromise({ try: () => ConfigMarkdown.parse(skillFile), catch: (error) => error }).pipe(
            Effect.catch(() => Effect.succeed(undefined)),
          )
          if (!md || typeof md.data !== "object" || md.data === null || typeof (md.data as { name?: unknown }).name !== "string") {
            skipped.push({ path: path.relative(dotDir, skillFile), reason: "invalid skill frontmatter" })
            continue
          }
          const data = md.data as { name: string; description?: string }
          const resources = yield* Effect.promise(() => collectSkillResources(path.dirname(skillFile)))
      ...
      if (mcpSvc) {
        for (const file of ["opencode.json", "opencode.jsonc"]) {
          ...
          const raw = yield* Effect.promise(() => Filesystem.readText(mcpPath).catch(() => ""))
          if (!raw) continue
          const data = yield* Effect.try({ try: () => ConfigParse.jsonc(raw, mcpPath), catch: (error) => error }).pipe(
            Effect.catch((error) =>
              Effect.sync(() => {
      ...
      if (toolSvc) {
        ...
        for (const toolFile of toolFiles) {
          ...
          const name = path.basename(toolFile, path.extname(toolFile))
          const raw = yield* Effect.promise(() => Filesystem.readText(toolFile).catch(() => ""))
          if (!raw.trim()) {
      ...
      if (commandSvc) {
        const commands = yield* Effect.tryPromise({
          try: () => ConfigCommand.load(dotDir),
          catch: (error) => error,
        }).pipe(
          Effect.catch((error) =>
            Effect.sync(() => {
              skipped.push({ path: "commands", reason: `failed to load commands: ${String(error)}` })
              return {} as Record<string, ConfigCommandV1.Info>
            }),
          ),
        )
        for (const [name, value] of Object.entries(commands)) {
          yield* commandSvc.upsert(sessionID, toCommandInput(name, value))
      ...
      // plugins
      if (pluginSvc) {
        const pluginUrls = yield* Effect.tryPromise({
          try: () => ConfigPlugin.load(dotDir),
          catch: (error) => error,
        }).pipe(
          Effect.catch((error) =>
            Effect.sync(() => {
              skipped.push({ path: "plugins", reason: `failed to load plugins: ${String(error)}` })
              return [] as ConfigPluginV1.Spec[]
            }),
          ),
        )
        for (const spec of pluginUrls) {
          const filePath = fileURLToPath(ConfigPlugin.pluginSpecifier(spec))
          const name = path.basename(filePath, path.extname(filePath))
          const code = yield* Effect.promise(() => Filesystem.readText(filePath).catch(() => ""))
          if (!code.trim()) {
            skipped.push({ path: path.relative(dotDir, filePath), reason: "empty plugin file" })
            continue
          }
