      ...
      const client = yield* Effect.gen(function* () {
        ...
        for (let attempt = 0; attempt < 30; attempt++) {
          ...
          const result = yield* Effect.tryPromise(() => withTimeout(c.connect(transport), connectTimeout)).pipe(
            Effect.map(() => c),
            Effect.catch((error) =>
              Effect.logWarning("sandbox MCP connect retry failed", { key, sessionID, port, attempt: attempt + 1, error: error instanceof Error ? error.message : String(error) }).pipe(
                Effect.as(undefined as MCPClient | undefined),
              ),
            ),
          )
        ...
        yield* Effect.logWarning("sandbox MCP connect exhausted all retries", { key, sessionID, port, attempts: 30 })
    ...
    const toolsForSession = Effect.fn("MCP.toolsForSession")(function* (sessionID: SessionID) {
      ...
      for (const [key, mcp] of Object.entries(config)) {
        ...
        const sandboxResult = yield* connectSandboxLocal(key, mcp as ConfigMCPV1.Info & { type: "local" }, sessionID, port).pipe(
          Effect.catch(() =>
            Effect.logWarning("config-level sandbox MCP failed", { key, sessionID, port }).pipe(
              Effect.as({ client: undefined, defs: undefined } as { client: MCPClient | undefined; defs: MCPToolDef[] | undefined }),
            ),
          ),
        )
        ...
        for (const row of sessionMcps) {
          ...
          if (row.type === "remote" && row.url) {
            ...
            const { client } = yield* connectRemote(row.name, remoteCfg).pipe(
              Effect.catch(() =>
                Effect.logWarning("session remote MCP failed", { name: row.name, sessionID }).pipe(
                  Effect.as({ client: undefined, status: { status: "failed", error: "" } } as { client: MCPClient | undefined; status: any }),
                ),
              ),
            )
            if (!client) continue
            ...
            const sandboxResult = yield* connectSandboxLocal(row.name, localCfg, sessionID, port).pipe(
              Effect.catch(() =>
                Effect.logWarning("session sandbox MCP failed", { name: row.name, sessionID, port }).pipe(
                  Effect.as({ client: undefined, defs: undefined } as { client: MCPClient | undefined; defs: MCPToolDef[] | undefined }),
                ),
              ),
            )
