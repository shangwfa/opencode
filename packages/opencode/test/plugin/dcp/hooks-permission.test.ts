...
test("system prompt handler injects nudges for main session with bundled internal prompts", async () => {
  const state = createSessionState()
  const handler = createSystemPromptHandler(state, new Logger(false), buildConfig("allow"), buildPromptStore())
  const output = {
    system: [
...
test("chat message transform replaces inputs for pruned error tools", async () => {
  const state = createSessionState()
  state.sessionId = "session-1"
  state.prune.tools.set("call-error", 42)
  const handler = createChatMessageTransformHandler(
    { session: { get: async () => ({}) } } as any,
    state,
    new Logger(false),
  ...
  const output = {
    messages: [
      buildMessage("user-1", "user", "retry the failed command"),
      {
        info: {
          id: "assistant-1",
          role: "assistant",
        ...
        parts: [
          {
            ...
            state: {
              status: "error",
              input: { command: "cat /workspace/missing.txt", timeout: "30000" },
  ...
  assert.equal(part.state.input.command, "[input removed due to failed tool call]")
  assert.equal(part.state.input.timeout, "[input removed due to failed tool call]")
  assert.equal(part.state.error, "file not found")
})
