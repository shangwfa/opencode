...
describe("session.user-fields message-v2 schema", () => {
  test("User schema accepts userName and userId", () => {
    const decoded = Schema.decodeUnknownSync(MessageV2.User)({
  ...
  test("User schema round-trips userName and userId through encoding", () => {
    ...
    const encoded = Schema.encodeSync(MessageV2.User)(input)
    const decoded = Schema.decodeUnknownSync(MessageV2.User)(encoded)
